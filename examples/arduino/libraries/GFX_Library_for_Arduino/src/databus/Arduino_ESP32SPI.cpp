...
{
#if defined(ESP_ARDUINO_VERSION) && defined(ESP_ARDUINO_VERSION_VAL) && (ESP_ARDUINO_VERSION >= ESP_ARDUINO_VERSION_VAL(3, 3, 0))
...
static void _on_apb_change(void *arg, apb_change_ev_t ev_type, uint32_t old_apb, uint32_t new_apb)
{
  ...
  else
  {
    ...
    _spi->dev->clock.val = gfxSpiFrequencyToClockDiv(_spi, freq);
    SPI_MUTEX_UNLOCK();
  }
}
...
bool Arduino_ESP32SPI::begin(int32_t speed, int8_t dataMode)
{
  ...
  spi_t *div_spi = &_spi_bus_array[_spi_num];
  if (!_div)
  {
    _div = gfxSpiFrequencyToClockDiv(div_spi, _speed);
  }
